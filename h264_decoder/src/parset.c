...
int ProcessSPS(seq_parameter_set_rbsp_t *SPS, NALU_t* nalu, unsigned char *nalu_buf, unsigned long int *nalu_bit_offset)
{
  ...
  (*nalu_bit_offset)=8;
  profile_idc_tmp=u_8(nalu, nalu_buf, nalu_bit_offset);

  c_set_flgs=u_8(nalu, nalu_buf, nalu_bit_offset);
  lvl_idc_tmp=u_8(nalu, nalu_buf, nalu_bit_offset);
  SPS_id=u_e(nalu, nalu_buf, nalu_bit_offset);
  ...
  SPS->log2_max_frame_num_minus4=u_e(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
  ...
  SPS->pic_order_cnt_type=u_e(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
  ...
  if(SPS->pic_order_cnt_type)
  {
    ...
    {
      SPS->offset_for_ref_frame[i]=s_e(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
  ...
  {
    SPS->log2_max_pic_order_cnt_lsb_minus4=u_e(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
  ...
  SPS->max_num_ref_frames=u_e(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
  fprintf(trace_bit,"%s %*d\n","SPS->max_num_ref_frames",50-strlen("SPS->max_num_ref_frames"),SPS->max_num_ref_frames);
#endif // _N_HLS_
  SPS->gaps_in_frame_num_value_allowed_flag=u_1(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
...
#endif // _N_HLS_
  SPS->pic_width_in_mbs_minus1=u_e(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
  fprintf(trace_bit,"%s %*d\n","SPS->pic_width_in_mbs_minus1",50-strlen("SPS->pic_width_in_mbs_minus1"),SPS->pic_width_in_mbs_minus1);
#endif // _N_HLS_
  SPS->pic_height_in_map_units_minus1=u_e(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
  ...
  SPS->frame_mbs_only_flag=u_1(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
  fprintf(trace_bit,"%s %*d\n","SPS->frame_mbs_only_flag",50-strlen("SPS->frame_mbs_only_flag"),SPS->frame_mbs_only_flag);
#endif // _N_HLS_
  if(!SPS->frame_mbs_only_flag)
  {
    SPS->mb_adaptive_frame_field_flag=u_1(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
  ...
  SPS->vui_parameters_present_flag=u_1(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
...
int ProcessPPS(pic_parameter_set_rbsp_t *pps, NALU_t* nalu, unsigned char *nalu_buf, unsigned long int *nalu_bit_offset)
{
  int pps_id;
  (*nalu_bit_offset)=8;
  pps_id=u_e(nalu, nalu_buf, nalu_bit_offset);
  ...
  pps->seq_parameter_set_id=u_e(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
  ...
  pps->entropy_coding_mode_flag=u_1(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
  fprintf(trace_bit,"%s %*d\n","pps->entropy_coding_mode_flag",50-strlen("pps->entropy_coding_mode_flag"),pps->entropy_coding_mode_flag);
#endif // _N_HLS_
  pps->bottom_field_pic_order_in_frame_present_flag=u_1(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
...
#endif // _N_HLS_
  pps->constrained_intra_pred_flag=u_1(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
  fprintf(trace_bit,"%s %*d\n","pps->constrained_intra_pred_flag",50-strlen("pps->constrained_intra_pred_flag"),pps->constrained_intra_pred_flag);
#endif // _N_HLS_
  pps->redundant_pic_cnt_present_flag=u_1(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
...
void process_pred_weight_table(pred_weight_table_rbsp_t* PWT, char B_slice, pic_parameter_set_rbsp_t* pps, NALU_t *nalu, unsigned char *nalu_buf, unsigned long int *nalu_bit_offset)
{
  PWT->luma_log2_weight_denom=u_e(nalu, nalu_buf, nalu_bit_offset);
  PWT->chroma_log2_weight_denom=u_e(nalu, nalu_buf, nalu_bit_offset);
  unsigned i;
  for(i=0;i<=pps->num_ref_idx_l0_active_minus1;i++)
  {
    PWT->luma_weight_l0_flag[i]=u_1(nalu, nalu_buf, nalu_bit_offset);
    if(PWT->luma_weight_l0_flag[i])
    {
      PWT->luma_weight_l0[i]=s_e(nalu, nalu_buf, nalu_bit_offset);
      PWT->luma_offset_l0[i]=s_e(nalu, nalu_buf, nalu_bit_offset);
    }
    PWT->chroma_weight_l0_flag[i]=u_1(nalu, nalu_buf, nalu_bit_offset);
    if(PWT->chroma_weight_l0_flag[i])
    {
      PWT->chroma_weight_l0[i][0]=s_e(nalu, nalu_buf, nalu_bit_offset);
      PWT->chroma_offset_l0[i][0]=s_e(nalu, nalu_buf, nalu_bit_offset);
      PWT->chroma_weight_l0[i][1]=s_e(nalu, nalu_buf, nalu_bit_offset);
      PWT->chroma_offset_l0[i][1]=s_e(nalu, nalu_buf, nalu_bit_offset);
    }
  }
  if(B_slice)
    return;
  ...
  {
    PWT->luma_weight_l1_flag[i]=u_1(nalu, nalu_buf, nalu_bit_offset);
    if(PWT->luma_weight_l1_flag[i])
    {
      PWT->luma_weight_l1[i]=s_e(nalu, nalu_buf, nalu_bit_offset);
      PWT->luma_offset_l1[i]=s_e(nalu, nalu_buf, nalu_bit_offset);
    }
    PWT->chroma_weight_l1_flag[i]=u_1(nalu, nalu_buf, nalu_bit_offset);
    if(PWT->chroma_weight_l1_flag[i])
    {
      PWT->chroma_weight_l1[i][0]=s_e(nalu, nalu_buf, nalu_bit_offset);
      PWT->chroma_offset_l1[i][0]=s_e(nalu, nalu_buf, nalu_bit_offset);
      PWT->chroma_weight_l1[i][1]=s_e(nalu, nalu_buf, nalu_bit_offset);
      PWT->chroma_offset_l1[i][1]=s_e(nalu, nalu_buf, nalu_bit_offset);
    }
  }
...
void process_dec_ref_pic_marking(dec_ref_pic_marking_rbsp_t *drpm, ImageParameters *img, NALU_t* nalu, unsigned char *nalu_buf, unsigned long int *nalu_bit_offset /* some more argument may be involved*/ )
{
  int i;
  if(nalu->nal_unit_type==5)
  {
    drpm->no_output_of_prior_pics_flag=u_1(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
    fprintf(trace_bit,"%s %*d\n","drpm->no_output_of_prior_pics_flag",50-strlen("drpm->no_output_of_prior_pics_flag"),drpm->no_output_of_prior_pics_flag);
#endif // _N_HLS_

    drpm->long_term_reference_flag=u_1(nalu, nalu_buf, nalu_bit_offset);
  }
  else
  {

    drpm->adaptive_ref_pic_marking_mode_flag=u_1(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
    ...
    if(drpm->adaptive_ref_pic_marking_mode_flag)
    {
      for(i=0;i<10;i++)
      {
        drpm->Memory_control_operation[i]=u_e(nalu, nalu_buf, nalu_bit_offset);
        if(drpm->Memory_control_operation[i])
          drpm->operation_value[i]=u_e(nalu, nalu_buf, nalu_bit_offset);
        else
        {
          break;
...
void ProcessSH(
    ...
    ImageParameters *img,
    NALU_t* nalu, unsigned char *nalu_buf, unsigned long int *nalu_bit_offset)
{
  (*nalu_bit_offset)=8;
  int i;

  SH->first_mb_in_slice=u_e(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
  fprintf(trace_bit,"%s %*d\n","SH->first_mb_in_slice",50-strlen("SH->first_mb_in_slice"),SH->first_mb_in_slice);
#endif // _N_HLS_

  SH->slice_type=u_e(nalu, nalu_buf, nalu_bit_offset)%5;
#if _N_HLS_
  fprintf(trace_bit,"%s %*d\n","SH->slice_type",50-strlen("SH->slice_type"),SH->slice_type);
#endif // _N_HLS_

  SH->pic_parameter_set_id=u_e(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
  fprintf(trace_bit,"%s %*d\n","SH->pic_parameter_set_id",50-strlen("SH->pic_parameter_set_id"),SH->pic_parameter_set_id);
#endif // _N_HLS_
  SH->frame_num=u_n(active_sps->log2_max_frame_num_minus4+4,nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
  ...
  if(img->IdrPicFlag)
  {
    SH->idr_pic_id=u_e(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
  ...
  {
    SH->pic_order_cnt_lsb=u_n(active_sps->log2_max_pic_order_cnt_lsb_minus4+4,nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
  ...
  //	{
  //		SH->redundant_pic_cnt=u_e(nalu, nalu_buf, nalu_bit_offset);
  //	}

  if(SH->slice_type==SLICE_TYPE_B)
  {
    SH->direct_spatial_mv_pred_flag=u_1(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
  ...
  {
    SH->num_ref_idx_active_override_flag=u_1(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
    ...
    {
      SH->num_ref_idx_l0_active_minus1=u_e(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
      fprintf(trace_bit,"%s %*d\n","SH->num_ref_idx_l0_active_minus1",50-strlen("SH->num_ref_idx_l0_active_minus1"),SH->num_ref_idx_l0_active_minus1);
#endif // _N_HLS_
      if(SH->slice_type==SLICE_TYPE_B)
      {
        SH->num_ref_idx_l1_active_minus1=u_e(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
  ...
  if(SH->slice_type!=2)
  {
    SH->ref_pic_list_modification_flag_l0=u_1(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
    ...
    if(SH->ref_pic_list_modification_flag_l0)
    {
      ...
      {
        SH->modification_of_pic_nums_idc_l0[i]=u_e(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
        ...
        {
          SH->modification_value_l0[i]=u_e(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
  ...
  if(SH->slice_type==1)
  {
    ...
    SH->ref_pic_list_modification_flag_l1=u_1(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
    ...
    if(SH->ref_pic_list_modification_flag_l1)
    {
      for(i=0;i<MAX_REFERENCE_PICTURES;i++)
      {
        SH->modification_of_pic_nums_idc_l1[i]=u_e(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
        ...
        {
          SH->modification_value_l1[i]=u_e(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
  ...
  {
    process_pred_weight_table(&(SH->pred_weight_table),  SH->slice_type-SLICE_TYPE_B,active_pps, nalu, nalu_buf, nalu_bit_offset);
  }

  if(nalu->nal_reference_idc !=0)
  {
    process_dec_ref_pic_marking(&(SH->dec_ref_pic_marking), img, nalu, nalu_buf, nalu_bit_offset /* some more argument may be involved*/ );
  }

  // if(active_pps->entropy_coding_mode_flag && SH->slice_type != SLICE_TYPE_I)
  // {
  // 	SH->cabac_init_idc=u_e(nalu, nalu_buf, nalu_bit_offset);
  // }


  SH->slice_qp_delta=s_e(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
