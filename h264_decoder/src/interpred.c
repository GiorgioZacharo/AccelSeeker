...
{
  unsigned char substepx[2][2] = { { 0 } };
  unsigned char substepy[2][2] = { { 0 } };
  unsigned char stepx = 0;
  unsigned char stepy = 0;
  unsigned char listuse[2][2] = { { 0 } };
  unsigned char preddir[2][2] = { { 0 } };
  ...
  if(MbType==5 || MbType==29)
  {
    ...
    for(i=0;i<2;i++)
      for(j=0;j<2;j++)
      {
        subtype=u_e(nalu, nalu_buf, nalu_bit_offset)+slice_type*4;
    ...
    if(num_ref_idx_active0>0 )
    {
      for(y=0;y<2;y+= (stepy/2) )
        for(x=0;x<2;x+= (stepx/2) )
        {
          ...
          {
            refidx=t_e(num_ref_idx_active0,nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
    ...
    if(num_ref_idx_active1>0  )
    {
      for(y=0;y<2;y+= (stepy/2) )
        for(x=0;x<2;x+= (stepx/2) )
        {
          ...
          {
            refidx=t_e(num_ref_idx_active1,nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
  ...
  unsigned char stpw = 0;
  unsigned char stph = 0;
      ...
      else if( listuse[x>>1][y>>1] & 0x01 )
      {
        ...
        for(suby=0;suby<stepy;suby+=stph)
          for(subx=0;subx<stepx;subx+=stpw)
          {
            ...
            if(skipflag!=1  )
            {
              mvdx=s_e(nalu, nalu_buf, nalu_bit_offset);
              mvdy=s_e(nalu, nalu_buf, nalu_bit_offset);
#if _N_HLS_
...
{
  int idxC[2] = { 0 };
  char refA0 = 0, refA1 = 0;
  char refB0 = 0, refB1 = 0;
  char refC0 = 0, refC1 = 0;
...
#pragma SDS data access_pattern(Schroma0:RANDOM)
#pragma SDS data mem_attribute(Schroma0:PHYSICAL_CONTIGUOUS)
#pragma SDS data zero_copy(Schroma0[0:PicWidthInSamplesC][0:FrameHeightInSampleC])

#pragma SDS data access_pattern(Schroma1:RANDOM)
#pragma SDS data mem_attribute(Schroma1:PHYSICAL_CONTIGUOUS)
#pragma SDS data zero_copy(Schroma1[0:PicWidthInSamplesC][0:FrameHeightInSampleC])

#pragma SDS data access_pattern(Schroma_cur:RANDOM)
#pragma SDS data mem_attribute(Schroma_cur:PHYSICAL_CONTIGUOUS)
#pragma SDS data zero_copy(Schroma_cur[0:PicWidthInSamplesC][0:FrameHeightInSampleC])
...
#pragma SDS data access_pattern(Sluma0:RANDOM)
#pragma SDS data mem_attribute(Sluma0:PHYSICAL_CONTIGUOUS)
#pragma SDS data zero_copy(Sluma0[0:PicWidthInSamplesL][0:FrameHeightInSampleL])

#pragma SDS data access_pattern(Sluma1:RANDOM)
#pragma SDS data mem_attribute(Sluma1:PHYSICAL_CONTIGUOUS)
#pragma SDS data zero_copy(Sluma1[0:PicWidthInSamplesL][0:FrameHeightInSampleL])

#pragma SDS data access_pattern(Sluma_cur:SEQUENTIAL)
#pragma SDS data mem_attribute(Sluma_cur:PHYSICAL_CONTIGUOUS)
#pragma SDS data zero_copy(Sluma_cur[0:PicWidthInSamplesL][0:FrameHeightInSampleL])
