...
int GetAnnexbNALU (NALU_t *nalu, unsigned char *nalu_buf, unsigned long int *nalu_bit_offset,FILE* bits)
{
  ...
  while (!StartCodeFound)
  {
    if (feof (bits))
    {
      ...
      memcpy (nalu_buf, &Buf[LeadingZero8BitsCount+nalu->startcodeprefix_len], nalu->len);
      nalu->forbidden_bit = (nalu_buf[0]>>7) & 1;
      nalu->nal_reference_idc = (nalu_buf[0]>>5) & 3;
      nalu->nal_unit_type = (nalu_buf[0]) & 0x1f;
...
void RBSPtoSODB(NALU_t *nalu, unsigned char *nalu_buf, unsigned long int *nalu_bit_offset, int len)
{
  unsigned char i;

  for(i=0;i<8;i++)
  {
    if( nalu_buf[len] & 0x01<<i)
    {
      break;
    }
  }
...
unsigned char more_rbsp_data(NALU_t *nalu, unsigned char *nalu_buf, unsigned long int *nalu_bit_offset)
{
  return (*nalu_bit_offset) < nalu->bit_length;
}
